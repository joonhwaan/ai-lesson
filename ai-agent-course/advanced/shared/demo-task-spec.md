...
> 세션 2(raw API) → 세션 6(LangChain) → 세션 7(LangGraph)에서 **같은 태스크가 진화**하는 모습을 보여준다.
...
- 입력: `{ "url": string }`
- 출력: `{ "text": string }`
- 용도: ReAct 다단계 추론(세션 3), 멀티에이전트 리서치(세션 8) 시연
...
- 입력: `{ "text": string }`
- 출력: `{ "summary": string }`
- 용도: 메모리·컨텍스트 관리(세션 4), 멀티에이전트 합성(세션 8)

#### 5. `cite`
- 설명: "주장과 출처를 매핑해 인용 항목을 만든다"
- 입력: `{ "claim": string, "source": string }`
- 출력: `{ "citation": string }`
- 용도: 추적성(traceability) 루브릭 — 멀티에이전트(세션 8), 평가(세션 9)

## 표준 시나리오 (강의 중 trace로 따라갈 입력)
> "2024년 노벨 물리학상 수상자가 누구이고, 그 상금을 3으로 나누면 얼마야?"
