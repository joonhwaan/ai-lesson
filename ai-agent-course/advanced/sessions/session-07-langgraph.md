# 세션 07: LangGraph — 상태 그래프 기반 에이전트
...
| 구간 | 시간 | 내용 |
|------|------|------|
| 복습 | 5분 | 세션 06: LCEL은 한 방향 파이프 → 루프 표현의 한계 |
...
세션 06에서 LCEL 체인 `prompt | llm | parser` 는 **한 방향 파이프**라고 했다. 하지만 우리 리서치 어시스턴트는 본질적으로 **루프**다.
...
직선 파이프(`|`)로는 못 그린다. 세션 06의 `AgentExecutor`는 이 루프를 **블랙박스 안에** 숨겼다. LangGraph는 그 루프를 **밖으로 꺼내 그래프로 명시**한다.
...
컴파일하면 그래프가 **Runnable**이 된다(세션 06의 인터페이스 재등장). 그래서 `.invoke`/`.stream`을 그대로 쓴다.
...
```
   세션 02: raw while 루프              세션 07: LangGraph 그래프
   ────────────────────────            ──────────────────────────
...
| 세션 02 (제어가 코드 안에 숨음) | 세션 07 (제어가 그래프로 드러남) |
|--------------------------------|----------------------------------|
...
세션 08에서는 단일 에이전트 그래프를 넘어, **여러 에이전트가 협업**하는 Multi-Agent 시스템(슈퍼바이저/서브그래프)을 다룬다. 노드 하나가 또 다른 그래프가 된다.
