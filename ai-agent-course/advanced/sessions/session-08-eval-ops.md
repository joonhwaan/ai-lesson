...
> 마지막 8번째 세션. 지금까지 만든 **리서치 어시스턴트(단일 → 멀티 에이전트)**를 어떻게 평가하고, 관찰하고, 프로덕션에 안전하게 올릴지 다룬다. 그리고 전체 과정을 회고한다.
...
| 흐름 | 라우팅 결정, 도구 호출 시퀀스 | 디버깅(7회차 멀티 에이전트) |
...
> 7회차의 "멀티 에이전트 디버깅 난이도"의 해답이 바로 이 tracing이다. 어느 worker가 비용을 쓰고 어디서 틀렸는지 span으로 본다.
...
S5 LangChain → 프레임워크 추상화 (Models/Prompts/Tools/Parsers, LCEL)
S6 LangGraph → 상태 그래프화 (노드/엣지/조건분기)
S7 멀티      → Supervisor + worker 오케스트레이션
S8 운영      → 평가 · 관찰가능성 · 프로덕션  ← 지금
```
...
| 구조 | S5–S6 | LangChain 추상화 → LangGraph 상태 그래프 |
| 협업 | S7 | 단일 vs 멀티, 오케스트레이션 패턴 |
| 운영 | S8 | 평가·관찰·가드레일·프로덕션 |
...
| 2. 도구 명세 | 도구 2개 이상, 시그니처·권한 | S2, S8-안전 |
| 3. 루프/구조 | ReAct? LangGraph 상태? | S3, S6 |
| 4. 메모리 | 무엇을 기억/요약하나 | S4 |
| 5. (선택) 검색/멀티 | RAG 또는 멀티 에이전트 필요성 논증 | S4(RAG), S7 |
| 6. 평가 루프 | 어떤 방법론 3종을 어떻게 적용 | S8 |
| 7. 가드레일 | 인젝션·권한·승인 게이트 | S8 |
| 8. 비용/실패 | 예상 비용, 재시도·폴백 | S8 |
...
→ 비용·지연·에러율 + 기본 trace부터 시작하면 충분하다. 멀티 에이전트(S7)로 가는 순간 tracing은 선택이 아니라 **필수**가 된다 — 어느 에이전트가 비용을 쓰고 어디서 틀렸는지 trace 없이는 알 수 없다.
...
> [!IMPORTANT] 마무리 메시지
> 8주 과정을 마칩니다. 핵심은 하나입니다 — ==에이전트는 "더 똑똑한 프롬프트"가 아니라, 추론·기억·도구·평가·운영이 맞물린 시스템==입니다. 캡스톤 설계서로 여러분의 첫 시스템을 직접 그려보세요. Q&A를 받습니다.
