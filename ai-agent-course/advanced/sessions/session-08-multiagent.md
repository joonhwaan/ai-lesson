# 세션 08 — 멀티 에이전트 시스템 & 오케스트레이션

> 여덟 번째 세션. 지금까지 단일 에이전트로 키워온 **리서치 어시스턴트**를 여러 에이전트가 협업하는 구조로 확장한다.
...
| 디버깅 난이도 | 어느 에이전트가 틀렸는지 불명확 | 결과만 보고 원인 추적 불가 | 전 단계 tracing(9회차), 에이전트별 로그 |
...
4·5회차에서 다룬 RAG(메모리로서의 검색증강과 그 구현·운용)를 멀티 에이전트와 결합하면, **검색 소스별로 전문 에이전트**를 둘 수 있다.
...
→ 맞다. Supervisor의 라우팅 오류는 흔한 실패 모드다. (1) 라우팅을 function calling으로 강제해 출력 형식을 고정, (2) 허용 목적지를 enum으로 제한, (3) step 상한과 FINISH 강제 종료로 폭주 방지. 9회차 평가·tracing에서 이 라우팅 정확도를 측정하는 법을 다룬다.

> 다음 시간(S9): 이렇게 복잡해진 멀티 에이전트를 **어떻게 평가하고, 관찰하고, 프로덕션에 올릴 것인가**를 다룬다.
