...
- **부분 실행/추적** — LangSmith로 각 단계 입출력 추적 (세션 08 예고)
- **타입 검사** — 단계 간 입출력 불일치를 조기에 발견

> [!WARNING] LCEL은 한 방향 파이프
> LCEL 체인은 "한 방향 파이프"다. 세션 02의 ==루프(반복·조건분기)== 는 단순 `|`로 표현되지 않는다. 그래서 LangChain은 에이전트 실행을 위해 `AgentExecutor`(혹은 더 명시적인 LangGraph)를 별도로 둔다. → 세션 06으로 이어짐.
...
### 6.4 세션 02 ↔ 세션 05 대응표 (꼭 칠판에 그릴 것)

| 세션 02 (raw 루프) | 세션 05 (LangChain) |
|--------------------|----------------------|
...
세션 06에서는 LCEL의 한 방향 파이프로는 표현 못 했던 **루프·조건분기**를 정면으로 다룬다. 같은 리서치 어시스턴트를 **LangGraph 상태 그래프**로 다시 그린다.
