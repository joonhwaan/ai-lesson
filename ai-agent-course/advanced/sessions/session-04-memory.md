...
5. 상태(state) 관리가 왜 메모리의 토대인지 이해하고, 세션 06 LangGraph state와 연결지을 수 있다.
...
| 개념 강의 ⑤ | 5분 | 상태(state) 관리와 세션 06 복선 |
...
> 지금은 "메모리 = state를 읽고 쓰는 규칙"으로 추상화해두면, 세션 06에서 코드로 자연히 연결된다.
...
- ✅ 상태(state) 관리가 세션 06 LangGraph state로 이어짐을 안다

---

## 8. 다음 시간 예고

세션 05 **LangChain 아키텍처 이해** — 세션 02에서 손으로 짠 raw 루프와 지금까지의 도구·메모리를
프레임워크의 표준 추상(Models/Prompts/Tools/Output Parsers, LCEL)으로 다시 그립니다.
(이후 세션 06에서 state/메모리를 LangGraph 상태 그래프로 통합)
