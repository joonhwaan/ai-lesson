...
여기까지 오느라 정말 고생 많으셨습니다. 👏 1부에서 **"AI가 왜 그렇게 동작하는가"**의 멘탈 모델을 쌓고, 2부에서 **"그래서 코딩 에이전트를 어떻게 활용하는가"**의 실전 감각을 익혔습니다. 이번 마지막 레슨은 그 두 부분을 하나의 흐름으로 잇고, 다음 무대인 **10세션 심화 과정**으로 가는 다리를 놓습니다.
...
| 08 코드베이스 이해 · 검증 중심 사고 | **세션 5** — RAG 심화(검색 증강 생성) |
| (추상화의 맛보기) | **세션 6·세션 7** — LangChain / LangGraph로 추상화 |
| 06 위임 · 다중 작업 | **세션 8** — 멀티 에이전트 & 오케스트레이션 |
| 02 환각/검증 · 11 리뷰/테스트 | **세션 9** — 평가·관찰가능성·가드레일·프로덕션 |
| 04 컨텍스트 · 07 하니스 | **세션 10** — 하네스 & 컨텍스트 엔지니어링 |
...
- 입문 과정 완료! 👏 이제 **10세션 심화 과정**(`../../advanced/site/index.html`)으로 이어가세요.
