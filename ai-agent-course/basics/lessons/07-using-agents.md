...
### 프롬프트를 구조화하는 4요소 — 목표 · 맥락 · 형식 · 예시

기존 패턴을 참조하라는 원칙을 좀 더 일반적인 ==프롬프트 뼈대==로 정리해 볼게요. 잘 쓴 프롬프트는 대개 네 가지를 담습니다. [[chip:info: 프롬프트 4요소]]

| 요소 | 무엇을 적나 | 예시 |
|------|-------------|------|
| **목표(Goal)** | 무엇을 이루려는지 한 문장으로 | "결제 실패 시 재시도 로직을 추가한다" |
| **맥락(Context)** | 관련 파일·제약·배경 | "`payment.ts`의 `charge()`를 쓰고, 최대 3회까지만" |
| **형식(Format)** | 결과물의 모양 | "함수 하나 + 단위 테스트, 주석은 한국어로" |
| **예시(Example)** | 본뜰 기존 코드/패턴 | "재시도 방식은 `retryFetch.ts`와 동일하게" |

네 요소를 다 채울 필요는 없어요. 하지만 결과가 자꾸 빗나간다면, ==빠진 요소가 무엇인지== 점검해 보세요. 대개 **맥락이나 형식**이 비어 있어서 에이전트가 추측하게 된 경우입니다.

> [!TIP] 프롬프트는 한 번에 완성되지 않는다
> 첫 프롬프트가 완벽할 필요는 없습니다. 결과를 보고 ==무엇이 어긋났는지==를 다음 프롬프트에 한 줄씩 더해 가는 **반복(iteration)**이 오히려 정석입니다. "방금 건 좋은데, 에러 처리만 `try/catch`로 바꿔줘"처럼 차이만 짚어 주면 됩니다.
